...
;// CONCATENATED MODULE: ./src/modules/apiRequest.ts
...
var baseApiURL = 'http://localhost:9090/api/';
function sendFormData(form) {
    return __awaiter(this, void 0, void 0, function () {
        var formData, requestBody, response;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4, fetch("".concat(baseApiURL, "registration"), {
                            method: 'POST',
                            body: requestBody,
                        })];
                case 1:
...
;// CONCATENATED MODULE: ./src/modules/formValidation.ts


var mask = '+{375} (00) 000-00-00';
...
var formValidation_form = document.forms[0];
var submitBtn = formValidation_form === null || formValidation_form === void 0 ? void 0 : formValidation_form.querySelector('.form__btn');
var inputErrorClassName = 'form__input--error';
var isValid = function () { return Array.from(formValidation_form.elements).every(function (el) { return !el.classList.contains(inputErrorClassName); }); };
function setInputMessage(el, message) {
    if (message === void 0) { message = ''; }
    message !== '' ? el.classList.add(inputErrorClassName) : el.classList.remove(inputErrorClassName);
    var messageContainer = el.parentElement;
    if (messageContainer) {
        var inputMessage = messageContainer.querySelector('.form__input-message');
        if (inputMessage)
...
function setFormMessage(type, message) {
    if (message === void 0) { message = ''; }
    var formMessage = formValidation_form === null || formValidation_form === void 0 ? void 0 : formValidation_form.querySelector('.form__message');
    if (formMessage) {
        formMessage.textContent = message;
        formMessage.className = 'form__message';
        if (type)
            formMessage.classList.add("form__message--".concat(type));
    }
}
function validateForm() {
    formValidation_form === null || formValidation_form === void 0 ? void 0 : formValidation_form.addEventListener('submit', function (e) {
        e.preventDefault();
        var target = e.target;
        var inputList = Array.from(target.elements);
        inputList.forEach(function (el) {
            if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) {
                var value = el.value, type = el.type, id = el.id, validity = el.validity;
                var message = '';
                if (!value.length)
        ...
        if (isValid()) {
            void sendFormData(formValidation_form)
                .then(function (response) {
                if (response.ok) {
                    formValidation_form.reset();
                }
                void response.json().then(function (data) { return setFormMessage(data.status, data.message); });
            })
...
;// CONCATENATED MODULE: ./src/modules/modalWindow.ts
...
function controlModal() {
    document.addEventListener('click', function (e) {
        var target = e.target;
        if (target.classList.contains('modal-control__btn'))
            showModal();
